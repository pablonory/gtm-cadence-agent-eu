...
> **UKI note (2026-09-01):** US-era build spec, kept as reference for the SHARED deployed app —
> flow names in here are the US set; the UKI set is `cadences/UKI_FLOWS.md` (unconfirmed).

The spec this app was built from. Tier: **Enterprise** (custom objects + permission sets confirmed
available). Nothing here sends outbound — the agent writes briefs; reps read them and assemble/activate
in Gong.

> **Status, 2026-08-24.** This is now a historical spec, kept because the code still cites its section
> numbers. It was written against an HTML prototype in `prototypes/`, which was deleted once the real app
> shipped. Where this file and the shipped app disagree, **the app wins** — see `README.md` for what is
> actually deployed. Two known divergences: §A2 lists 20 properties where the live object has 36, and
> §B1 specifies a Private App token where the app ships static auth. Its §G open-questions list is stale;
> four of the eight were settled elsewhere.
...
- ~~Realign or retire `cadences/*.md` (9 cells) against Lewis's 16 US Flows~~ — **done 2026-08-24: retired.**
