...
OUTPUTS
  ...
  cadences/registry/asks/<rep>.md       a 2-minute, paste-able message for THAT rep — the only way to learn which
                                        flows are live (Gong exposes no usage data; verified 2026-09-11: the stats
                                        endpoints return call activity per user, nothing flow-level)
...
def write_ask(rep, personal, shared):
    """A 2-minute message for one rep. Gong cannot tell us which flows are live, so the rep does.

    Only plausible TEMPLATES are listed (outbound/reactivation with a vertical or persona tag);
    event, inbound and account-specific flows are counted but not listed — asking about "Boparan
    Outreach" wastes the rep's attention and the agent would never map to it anyway.
    """
    slug = rep["email"].split("@")[0]
    cand = [e for e in personal if e["tags"]["motion"] in ("outbound", "reactivation")
            and (e["tags"]["vertical"] or e["tags"]["persona"] or e["tags"]["motion"] == "reactivation")]
    other = len(personal) - len(cand)
    lines = [f"# Ask — {rep['name']} ({rep['email']})", "",
             "Paste into Slack. Regenerated by `scripts/gong_flows.py`; replies go into "
             f"`cadences/registry/{slug}.json` (`live: true/false`).", "", "---", ""]
    lines.append(f"Hi {rep['name'].split()[0]} — two minutes for the outbound agent, if you have them.")
    lines.append("")
    if cand:
        lines.append("These are the flows I can see under your name in Gong that look like reusable "
                     "templates. **Which of them do you still actually run?** Reply with the numbers "
                     "you'd keep — \"none\" is a perfectly good answer.")
        lines.append("")
        for i, e in enumerate(cand, 1):
            y, m, _ = e["created"].split("-")
            lines.append(f"{i}. **{e['name']}** — created {MONTHS[int(m)]} {y}")
        lines.append("")
    else:
        lines.append("I can't see any reusable templates under your name in Gong — which probably means "
                     "you run the shared flows, or yours live in a shared folder.")
        lines.append("")
    if other:
        lines.append(f"(I'm ignoring {other} event/one-off {'flow' if other == 1 else 'flows'} — Tech Expo, "
                     "named-account outreach and the like. Shout if one of those is actually your everyday flow.)")
        lines.append("")
    lines.append("Two more things:")
    lines.append("")
    lines.append("- If you run something that isn't on the list, tell me the name exactly as it appears in Gong.")
    lines.append("- Do you use the shared **Outbound** flows (Bar/Pub · Coffee · Resto × CEO / Finance / IT / "
                 "Ops / People, plus the SMB set)? If so, which ones?")
    lines.append("")
    lines.append("**Why I'm asking:** when the agent hands you an account brief it will name the Gong flow to "
                 "drop that account into. I want it naming *your* flows, not a generic set — and I'd rather ask "
                 "you than guess from flow names.")
    lines.append("")
    if shared:
        lines.append(f"---\n\n*Note for Pablo: {rep['name']} also sees {len(shared)} `Shared` flows. Gong does not "
                     "report who created a flow, so any of those could be theirs — worth a follow-up if the "
                     "answer above is thin.*")
    open(os.path.join(ASK_DIR, f"{slug}.md"), "w").write("\n".join(lines) + "\n")
    return len(cand)
...
def main():
    ...
    for rep in roster:
        ...
        usable = write_ask(rep, draft["personal"], shared)
        ...
        print(f"{slug:16} personal {len(personal):3} · template candidates {usable:3} · shared {len(shared):3}")
