...
OUTPUTS
  ...
  cadences/registry/REVIEW.md           master review sheet, all reps — COMMITTED, annotatable in the Obsidian vault
  cadences/registry/asks/<rep>.txt      the exact Slack message for THAT rep — four questions, pre-filled with the
                                        flows scripts/gong_flow_usage.py measured them editing, so question one is a
                                        confirmation rather than a memory test. Nothing else is in the file: sending
                                        is one call. Run gong_flow_usage.py FIRST so the usage is fresh.
...
ASK_DIR = os.path.join(ROOT, "cadences", "registry", "asks")
...
def entry(f, layer):
    ...
    return {"id": f["id"], "name": f["name"], "folder": (f.get("folderName") or "").strip() or None,
            ...
            "live": None}  # tri-state: true = rep confirmed · null = unconfirmed (matcher may SUGGEST it,
                           # the name is verbatim from their own Gong) · false = rep said no, never suggest


MONTHS = ["", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
USAGE = os.path.join(ROOT, "cadences", "registry", "_usage.json")


def load_usage():
    """rep email → [{flow, flow_id, touches, last_seen}], most recent first. Built by
    scripts/gong_flow_usage.py from the Gong audit log: flows the rep has EDITED. A proxy for
    'live', never proof — and no edits does not mean dead (see that script's docstring)."""
    if not os.path.isfile(USAGE):
        return {}, None
    d = json.load(open(USAGE))
    return d.get("by_rep", {}), d.get("window_days")
...
def write_ask(rep, personal, shared, usage, window):
    """The exact Slack message for one rep — nothing else in the file, so sending is copy-paste
    or one tool call. Questions are ordered cheapest-first: confirm measured usage, then recall.
    """
    slug = rep["email"].split("@")[0]
    first = rep["name"].split()[0]
    used = [u for u in usage.get(rep["email"], []) if u["touches"]][:8]
    used_ids = {u["flow_id"] for u in used}
    # templates the rep owns that the log did not see them touch
    cand = [e for e in personal
            if e["tags"]["motion"] in ("outbound", "reactivation")
            and (e["tags"]["vertical"] or e["tags"]["persona"] or e["tags"]["motion"] == "reactivation")
            and e["id"] not in used_ids]
    L = [f"Hi {first} — two minutes for the outbound agent, if you have them.", ""]
    q = 1
    if used:
        L += [f"*{q}. These are the flows you've actually touched in Gong in the last {window or 90} days. "
              "Which do you still run for new outbound?*", ""]
        L += [f"• {u['flow']}  _(last edited {nice_day(u['last_seen'])})_" for u in used]
        L += ["", "_Reply with the ones to keep — \"all\" or \"none\" are both fine answers._", ""]
        q += 1
    if cand:
        label = "You also have these older templates. Any of them still in use?" if used else \
                "These look like reusable templates under your name in Gong. Which do you still run?"
        L += [f"*{q}. {label}*", ""]
        L += [f"• {e['name'].strip()}  _(created {nice_day(e['created'])} {e['created'][:4]})_" for e in cand[:10]]
        L += [""]
        q += 1
    if not used and not cand:
        L += [f"*{q}. I can't see any reusable templates under your name in Gong* — so I'm guessing you "
              "mostly run the shared flows. Which ones?", ""]
        q += 1
    L += [f"*{q}. Anything you run that isn't listed above?* Give me the name exactly as it appears in Gong.", ""]
    q += 1
    L += [f"*{q}. Do you use the shared _Outbound_ flows?* (Bar/Pub, Coffee and Resto × CEO / Finance / IT / "
          "Ops / People, plus the SMB set.) If so, which.", ""]
    q += 1
    L += [f"*{q}. When an account lands on your desk, what decides which flow it goes into?* "
          "Vertical, persona, number of sites, something else?", ""]
    L += ["*Why I'm asking:* the agent is going to hand you a brief on an account with the Gong flow to drop "
          "it into. I want it naming _your_ flows and picking them the way you would, rather than a generic set.", ""]
    if shared:
        L += [f"_(Heads up: you can also see {len(shared)} flows shared by other people. Gong doesn't record who "
              "created a flow, so if one of those is really yours, say so.)_"]
    open(os.path.join(ASK_DIR, f"{slug}.txt"), "w").write("\n".join(L).rstrip() + "\n")
    return len(used), len(cand)
...
def main():
    ...
    auth = gong_auth()
    for d in (RAW_DIR, DRAFT_DIR, ASK_DIR, os.path.dirname(COMPANY_OUT)): os.makedirs(d, exist_ok=True)

    usage, window = load_usage()
    ...
    for rep in roster:
        ...
        n_used, n_cand = write_ask(rep, draft["personal"], shared, usage, window)
        ...
        print(f"{slug:16} personal {len(personal):3} · measured-in-use {n_used:3} · other templates {n_cand:3} · shared {len(shared):3}")
